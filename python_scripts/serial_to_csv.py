...
try:
    if PLATFORM == "Windows":
        ...
        for p in list(serial.tools.list_ports.comports()):
            print(p, end="")
            print(p.description)
            # make sure there is actually a manufacture infomation avaiable 
            # if not just skip that device
            
            if type(p.manufacturer) == "NoneType":
                break
    ...
    try:
        with serial.Serial(COM_PORT,9600) as ser:
            while True:
                ...
                print(raw_temp)
                raw_temp = raw_temp.split(":")
                raw_temp = raw_temp[-1].strip()
