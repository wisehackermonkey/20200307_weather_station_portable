...
void setup()
{
  ...
  Serial.println("When the temperature changes the led onboard should blink,");
  Serial.println("and print the temperature's value in C ");
...
void loop()
{
  //reading analog 0 for water sensor
  watersensor.update();
  sensor.update();
    // int temp  = -1;
  ...
  if (sensor.update())
  {
    ...
    // if( ((valuePrev >= temp)   && ((valuePrev - temp) > 1))  || ( (valuePrev < temp) && (temp - valuePrev) > 1 )  ){
    //   valuePrev = temp;
